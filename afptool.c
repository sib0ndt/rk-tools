...
static int copy_string_field(char *dst, size_t dst_len, const char *src)
{
...
int action_parse_key(char *key, char *value) {
	if (strcmp(key, "FIRMWARE_VER") == 0) {
		...
		if (copy_string_field(package_image.machine_model,
				sizeof(package_image.machine_model), value))
			return -1;
	} else if (strcmp(key, "MACHINE_ID") == 0) {
		if (copy_string_field(package_image.machine_id,
				sizeof(package_image.machine_id), value))
			return -1;
	} else if (strcmp(key, "MANUFACTURER") == 0) {
		if (copy_string_field(package_image.manufacturer,
				sizeof(package_image.manufacturer), value))
			return -1;
...
int append_package(const char *name, const char *path)
{
	...
	if (package_image.num_package >=
			sizeof(package_image.packages) / sizeof(package_image.packages[0]))
	{
		fprintf(stderr, "Too many package entries\n");
		return -1;
	}

	if (copy_string_field(p_pack->name, sizeof(p_pack->name), name) ||
		copy_string_field(p_pack->filename, sizeof(p_pack->filename), path))
	{
		fprintf(stderr, "Package entry too long: %s %s\n", name, path);
		return -1;
	}
