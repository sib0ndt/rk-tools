...
int parse_version_arg(const char *arg, unsigned long max, int *value)
{
	char *end;
	...
	if (errno || end == arg || *end != '\0' || parsed > max)
		return -1;
...
int main(int argc, char **argv)
{
	...
	if (argc == 8)
	{
		...
		if (parse_version_arg(argv[3], 0xff, &majver) ||
			parse_version_arg(argv[4], 0xff, &minver) ||
			parse_version_arg(argv[5], 0xffff, &subver))
		{
		...
		// loader, majorver, minorver, subver, oldimage, newimage
		return pack_rom(argv[2], majver, minver, subver, argv[6], argv[7]);
	}
	else
	{
		usage(argv[0]);
